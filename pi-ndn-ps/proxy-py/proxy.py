...
def run_data_fetcher(str):
    ...
    os.system("dc-exp " + str + " " + sys.argv[1] + " " + sys.argv[2])
...
class OnboardListener(object):
    ...
    def onInterest(self, prefix, interest, face, interestFilterId, filter):
        ...
        routeToRegister = str(Name(pubsubPrefix + "/" + repoName + "/" + deviceID))
        
        registerRouteWithNameAndIp(routeToRegister, deviceIP)

        thread = Thread(target=run_data_fetcher, args=(deviceID,))
...
def registerPrefixForSeqReq():
    ...
    prefix = Name(pubsubPrefix + "/" + repoName + "/proxy").append("initialSeq")
...
def registerPrefixForDeviceIDReq():
    ...
    prefix = Name(pubsubPrefix + "/" + repoName + "/proxy").append("deviceIDList")
...
repoCommandPrefix = Name("/localhost/" + repoName)
global repoDataPrefix
repoDataPrefix = Name(pubsubPrefix + "/" + repoName)
